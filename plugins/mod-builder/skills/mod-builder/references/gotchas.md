...
6. Anything but a literal `$.noun.verb(...)` fails validation. Verified on 2.1.272, three shapes, three messages. `$[n].read(...)` reads `a computed or optional member access on $`. `$.fs?.read(...)` reads `$.fs is used as a value (a noun of $ bound, passed or read)`. `const d = $` reads `$ itself is bound to a name (bound, passed, spread, returned or read)`. Each ends with the same reminder: `$ is always spelled $.noun.event(...) at the call site, on is always on("<event>", hook), and next.to always next.to(e, "<tier>")`. The engine inventories calls statically, so every call is spelled out in full.
...
11. The tsconfig that shipped mods use: `"jsx": "react"`, `"jsxFactory": "h"`, `"jsxFragmentFactory": "Fragment"`, `"types": []`, `"moduleResolution": "bundler"`, `"noEmit": true`, and `"include": [".claude/types", "hooks"]`. Source: cc-arcade `tsconfig.json`, https://github.com/sezaakgun/cc-arcade/blob/main/tsconfig.json. Anthropic's `mods/tsconfig.json` (https://github.com/anthropics/claude-code/blob/main/mods/tsconfig.json) uses the same `jsx`, `jsxFactory`, `jsxFragmentFactory`, `types`, `moduleResolution` and `noEmit` values, adds `noUncheckedIndexedAccess`, drops `allowImportingTsExtensions`, and includes `types`, `*/types/**/*.d.ts`, `*/hooks` and `*/tests`.
...
20. Core has a side effect on `tool.call`, `prompt.submit`, `prompt.fill`, `prompt.suggest`, `turn.step`, `session.compact`, `agent.spawn`, `command.run`, `config.set`, `ui.press`, `ui.input`, `ui.message`, `classic.*` and `*`. No `next` and it did not happen. `next` twice and it happened twice. Source: the filled markers on the cheat sheet's event table.
