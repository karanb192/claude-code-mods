...
Everything the skill relies on, with the URL. Fetch before trusting any shape;
the API is early access.

## Source of truth order

1. The current binary's `.claude/types/claude-code.d.ts`, generated by
   `/plugin-types` in the session where the Mod will run.
2. `claude plugin validate` on that Mod and binary.
3. Anthropic's current `mods/types/claude-code.d.ts` source.
4. The official cheat sheet and architecture paper attached to issue #91870.
5. Anthropic's built-in Mods and test kit.
6. Community Mods and the awesome catalogue, for examples and discovery only.

When a lower item conflicts with a higher item, state the conflict and follow
the higher item.
